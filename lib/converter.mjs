...
export function convertFileReferences(commentBody, prFiles) {
    ...
    // Extract and temporarily replace markdown links to avoid processing them
    const markdownLinks = [];
    const markdownLinkPlaceholder = '___MARKDOWN_LINK_PLACEHOLDER___';

    // Find all markdown links ([text](url))
    updatedCommentBody = updatedCommentBody.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match) => {
        markdownLinks.push(match);
        return `${markdownLinkPlaceholder}${markdownLinks.length - 1}`;
    });
    ...
    updatedCommentBody = updatedCommentBody.replace(new RegExp(`${markdownLinkPlaceholder}(\\d+)`, 'g'), (_, index) => {
